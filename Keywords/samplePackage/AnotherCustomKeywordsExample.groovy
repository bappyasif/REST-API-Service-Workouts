...
class AnotherCustomKeywordsExample {
	/**
	...
	/*
	 * This Function Here Simply Sends A Request And Verify Test Object Verification.
	 * This Will Also Log It's Entire Request Content Body, As A Whole.
	 */
	@Keyword
	def bookContains(TestObject testObject) {
		if(testObject instanceof RequestObject) {
			RequestObject reqObject = (RequestObject)testObject
			ResponseObject resObject = WSBuiltInKeywords.sendRequestAndVerify(reqObject)
			KeywordUtil.logInfo("Element Found : "+resObject.responseBodyContent)
			//def verifiedElement = resObject.responseBodyContent.PageCount
			//KeywordUtil.markPassed("Element Fopund : "+verifiedElement)
			
		} else {
			KeywordUtil.markError("Not Found!!")
		}
	}
	
	/*
	 * This Function Here Simply Sends A Request And Verify Test Object Verification.
	 * It's Also Checks Out Specific Element Verification, Here It's PageCount Value.
	 */
	@Keyword
	def bookSpeceficContentExtraction(TestObject testObject) {
		if(testObject instanceof RequestObject) {
			RequestObject reqObject = (RequestObject) testObject
			ResponseObject resObject = WSBuiltInKeywords.sendRequestAndVerify(reqObject)
			
			def jsonResponse = new groovy.json.JsonSlurper()
			def contentResponse = jsonResponse.parseText(resObject.responseBodyContent)
			
			def extractContent = contentResponse.PageCount
			KeywordUtil.logInfo("Extracted Content : "+extractContent)
			
			if (WS.verifyElementPropertyValue(resObject, 'PageCount', extractContent)) {
				KeywordUtil.markPassed("Content Mathes")
			} else {
				KeywordUtil.markErrorAndStop("Mis Match!!")
			}
		} else {
			KeywordUtil.markErrorAndStop("Not Found!!")
		}
	}
