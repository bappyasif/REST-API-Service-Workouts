...
class SampleCustomKeywordsExample {
	/**
	...
	/*
	 * This is function will have a cross check across range of status code values, from any valid test request
	 * When matches with checked codes a success message will show or error message.  
	 */
	@Keyword
	def successfulRequestStatusCode(TestObject testRequest) {

		if(testRequest instanceof RequestObject) {

			RequestObject requestObject = (RequestObject) testRequest
			ResponseObject responseObject = WSBuiltInKeywords.sendRequest(requestObject)

			//let arrayList = [200, 201, 202]

			if(responseObject.getStatusCode() == 200 || 201 || 202) {

				KeywordUtil.markPassed("Successfull request")

			} else {

				KeywordUtil.markError("Code Does Not Matches")

			}

		} else {

			KeywordUtil.markFailed(testRequest.getObjectId() + " is not a RequestObject")
	...
	/*
	 * This function will check whether response text does contain a sequence of character or not
	 * When it does them success message or otherwise error message will be shown. 
	 */
	@Keyword
	def responseContains(TestObject testRequest) {

		RequestObject requestObject = (RequestObject)testRequest
		ResponseObject responseObject = WSBuiltInKeywords.sendRequest(requestObject)

		//WS.verifyElementPropertyValue(responseObject, "PageCount", 200)
		if(responseObject.getResponseText().contains('Book')) {

			KeywordUtil.markPassed("Contains Element")
